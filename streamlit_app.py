...
# --- FIXED COLOR CHOICES (6 Clean Options without labels) ---
COLOR_PALETTE = {
    "🟡 #FAD02C": "#FAD02C",
    "🔵 #A2CFFE": "#A2CFFE",
    "🟢 #B2F7EF": "#B2F7EF",
...
# Helper function to get color key by its hex value
def get_color_key(hex_value):
    for key, hex_code in COLOR_PALETTE.items():
        if hex_code == hex_value:
            return key
    return "🟡 #FAD02C" # Fallback default
...
with st.sidebar:
    ...
    with tabs[0]:
        ...
        # Radio selector showing only color emojis and hex codes
        selected_color_key = st.radio("Select Box Color:", list(COLOR_PALETTE.keys()), key="new_box_color")
        add_color = COLOR_PALETTE[selected_color_key]
    ...
    with tabs[1]:
        if not events:
            ...
            current_color_key = get_color_key(selected_event["backgroundColor"])
            edit_color_key = st.radio("Modify Box Color:", list(COLOR_PALETTE.keys()), index=list(COLOR_PALETTE.keys()).index(current_color_key), key="edit_box_color")
            edit_color = COLOR_PALETTE[edit_color_key]
